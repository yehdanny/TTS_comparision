...
_WORKER_SCRIPT = str(Path(__file__).parent.parent / "workers" / "cosy_worker.py")
_PYTHON_EXE    = str(Path(__file__).parent.parent.parent / "venv_cosy" / "Scripts" / "python.exe")
...
class CosyVoiceModel(WorkerModel):
    def __init__(self):
        super().__init__(
            name="CosyVoice",
    ...
    def generate(self, text: str, ref_audio_path: str | None = None) -> dict:
        ...
        logger.warning("[CosyVoice] Worker not running — returning stub output")
        out_path = os.path.join(_OUTPUT_DIR, f"cosyvoice_{uuid.uuid4().hex}.mp3")
